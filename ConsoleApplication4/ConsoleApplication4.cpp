...
const double a = 8901500;
const double mu = 4.28214 * pow(10, 13);
...
int main()
{
	...
	for (int i = 0; i < 361; i++)
	{
		double teta = excentricToTrue(iterationMethod(e * sin(i * 2 * Pi / 360) + (i * 2 * Pi / 360), i * 2 * Pi /
			360, i * 2 * Pi / 360));
		std::cout << std::fixed << radFind(teta, p) << "\t" << speedRad(p, teta) << "\t" << speedN(p, teta) <<
			"\t" << fullSpeed(p, teta) << std::endl;
	}
	return 0;
}